...
public class MainRenderer implements GLSurfaceView.Renderer {
    ...
    private List<Sphere> mShperes = new ArrayList<Sphere>();
    ...
    public void onDrawFrame(GL10 gl10) {
        ...
        for (int i = 0; i < mShperes.size(); i++) {
            Sphere sphere = mShperes.get(i);
    ...
    public void updateViewMatrix(float[] matrix) {
        ...
        for (int i = 0; i < mShperes.size(); i++) {
            mShperes.get(i).setViewMatrix(matrix);
        }
    ...
    public int removePoint() {
        ...
        if (mShperes.size() >= 1) {
            mShperes.remove(mShperes.size() - 1);
        }
        if (mLines.size() >= 1) {
